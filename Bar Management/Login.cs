...
namespace Bar_Management
{
    public partial class Login : Form
    {
        ...
        {
            var checkTK = _logic.GetAll().FirstOrDefault(x => x.Ten.ToString()== username && x.MatKhau.ToString() == password);
            if (checkTK == null)
            {
                return null;
            }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                newForm.btnCategory.Enabled = false;
                newForm.btnEmployee.Enabled = false;
                newForm.btnRevenue.Enabled = false;
                newForm.btnOrderHistory.Enabled = false;
                newForm.btnAccount.Enabled = false;

                if (check.Role.ToString() == "Nhân viên kho") {
                    newForm.btnFood.Enabled = false;
                    newForm.btnOrder.Enabled = false;
                } else {
                    newForm.btnWarehouse.Enabled = false;
                    newForm.btnNhaCungCap.Enabled = false;
                }
